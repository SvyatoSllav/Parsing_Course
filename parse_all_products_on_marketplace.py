...
def get_every_pagination_page():
    """ Парсит каждую страницу в пагинации категорий. """
    all_categories_pages = [tag_a.get("href") for tag_a in index_page_soup.find("div", class_="nav_menu").find_all("a")]
    all_category_pagination_pages = []
    for category_page in all_categories_pages:
        category_page_response = requests.get(f"{schema}{category_page}")
        ...
        page_in_pagination = category_page_soup.find("div", class_="pagen").find_all("a")
        pagination_pages = [f"{schema}{tag['href']}" for tag in page_in_pagination]
        all_category_pagination_pages.extend(pagination_pages)
...
with open("all_products.csv", "w", encoding="utf-8-sig", newline="") as file:
    ...
    for category_page in get_every_pagination_page():
        ...
        for item_link in all_item_links:
            ...
            brand, model = description[0], description[1]
            ...
            flatten = name, article, brand, model, in_stock, price, old_price, item_link
